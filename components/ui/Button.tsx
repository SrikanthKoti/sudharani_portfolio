...
export function Button({
    ...
    // Variants WITHOUT any shadow classes
    const variants = {
        primary:
            "min-w-[180px] h-14 px-6 rounded-xl bg-primary text-white text-base",
        secondary:
    ...
    // Shadow styles applied ONLY when allowed
    const shadowStyles =
        !disableShadow && variant === "primary"
            ? "shadow-lg shadow-primary/25 hover:shadow-xl hover:shadow-primary/30 hover:-translate-y-1"
            : "";

    const width = fullWidth ? "w-full" : "";

    const combined = `${base} ${variants[variant]} ${shadowStyles} ${width} ${className}`;
